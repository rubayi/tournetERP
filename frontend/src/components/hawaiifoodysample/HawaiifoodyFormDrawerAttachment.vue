<template>
  <card-comp-design
    id="hawaiifoodydocumentform"
    bordered="false"
    title="Attachment"
  >
    <template #content>
      <div id="hawaiifoodydocument-grid-container" class="row grow-1">
        <div class="col-12">
              ...
              <tr v-for="item in attachmentlist" :key="item.fildid">
                ...
                <td class="text-left no-padding">
                  ...
                  >
                    <img
                      align="absmiddle"
                      :src="require(`@/assets/img/${item.filenameext}`)"
  ...
  <dialog-comp
    ...
    @confirm-clicked="deleteHawaiifoodydocumentForm"
  />
</template>
...
export default defineComponent({
  name: "HawaiifoodyFormDrawerResponse",
  components: {
    DialogComp,
    SelectComp,
    CardCompDesign,
  ...
  props: {
    ...
    attachmentlist: {
      type: Array as () => HawaiifoodyAttachmentForm[],
      default: () => [],
    },
  ...
  setup(props, { emit }) {
    ...
    const selectedEntry = ref<HawaiifoodyAttachmentForm>();
    ...
    function downloadAction(selectfile: any) {
      if (selectfile != 0) {
        window.location.href =
          "/hawaiisampledemo/hawaiifoody/attachmentdownload/" +
          selectfile;
      }
    }

    function deleteAction(selectfile: any) {
      editfileid.value = selectfile;
      openDeleteConfirm.value = true;
    }
    function deleteHawaiifoodydocumentForm() {
      loading.value = true;
      AttachmentService.deleteHawaiifoodydocumentForm(editfileid.value)
        .then(() => {
    ...
    function addAction(file: any) {
      const formData = new FormData();
      formData.append("uploadBtn", file[0]);
      notificationHelper.dismiss();
      notificationHelper.createOngoingNotification("Uploading...");
      loading.value = true;

      if (formData) {
        AttachmentService.saveHawaiifoodydocumentForm(
          props.subworknum,
          subattachtype.value,
          formData
        )
...
<style lang="scss">
#hawaiifoodydocument-grid-container {
  flex-grow: 1;
  .grid {
    height: 180px;
  }
